@extends('layouts.app')
@section('title')
    <a href="{{ url('/supir') }}" class="text-black-600 hover:text-blue-600 hover:none">Master Supir</a> > Ubah Data
@endsection
@section('subtitle', 'Halaman ini digunakan untuk melakukan ubah data supir dalam sistem')
@section('content')

<div class="p-3 md:p-6">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('supir.update', $data->supir_id) }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <select name="jns_sim_id" class="w-full rounded-md border border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200">
                    ...
                    @foreach($jenisSim as $sim)
                        <option value="{{ $sim->jns_sim_id }}" {{ old('jns_sim_id', $data->jns_sim_id) == $sim->jns_sim_id ? 'selected' : '' }}>{{ $sim->jns_sim_nama }}</option>
                    @endforeach
                </select>
            ...
            <div class="grid md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="driver_status" class="w-full rounded-md border border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200">
                        <option value="1" {{ old('driver_status', $data->driver_status) == '1' ? 'selected' : '' }}>Aktif</option>
                        <option value="2" {{ old('driver_status', $data->driver_status) == '2' ? 'selected' : '' }}>Tidak Aktif</option>
                    </select>
