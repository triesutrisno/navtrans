...
@section('subtitle', 'Halaman ini digunakan untuk mendefinisikan master role kedalam sistem')
@section('content')
...
<div class="p-3 md:p-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div class="flex items-center gap-2">
            ...
            <a href="{{ url('/mrole') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-yellow-500 hover:bg-yellow-600 text-white text-sm rounded-lg transition">
            ...
            <a href="{{ url('/mrole/create') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-sm rounded-lg transition">
                    <svg xmlns="http://www.w3.org/2000/svg"
                        class="w-4 h-4"
                        fill="none"
                        viewBox="0 0 24 24"
        ...
        <div class="w-full overflow-x-auto">
            <table class="min-w-full text-sm text-left whitespace-nowrap">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($data as $key => $datae)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 text-gray-500">{{ $data->firstItem() + $key }}</td>                           
                            <td class="px-6 py-4"><div class="px-6 py-4 text-xs">{{ $datae->role_nama }}</div></td>
                            <td class="px-6 py-4 text-xs">{{ $datae->role_keterangan }}</td>
                            <td class="px-6 py-4">
                                @if($datae->role_status == '1')
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-col md:flex-row justify-center gap-2">
                                    ...
                                    <a href="{{ url('/mrole/edit') }}/{{ $datae->role_nama }}"
                                    ...
                                    <a href="{{ route('mrole.delete', $datae->role_nama) }}"
