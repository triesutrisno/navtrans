...
@section('subtitle', 'Halaman ini digunakan untuk mendefinisikan master kota kedalam sistem')
@section('content')
...
<div class="p-3 md:p-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div class="flex items-center gap-2">
            ...
            <a href="{{ url('/kota') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-yellow-500 hover:bg-yellow-600 text-white text-sm rounded-lg transition">
            ...
            <a href="{{ url('/kota/create') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-sm rounded-lg transition">
                    <svg xmlns="http://www.w3.org/2000/svg"
                         class="w-4 h-4"
                         fill="none"
                         viewBox="0 0 24 24"
            ...
            <table class="min-w-full text-sm text-left whitespace-nowrap">
                <thead class="bg-gray-50 text-gray-600 uppercase text-xs">
                    <tr>
                        ...
                        <th class="px-6 py-4">Kode Kota</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($data as $key => $kota)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 text-gray-500">{{ $data->firstItem() + $key }}</td>                           
                            <td class="px-6 py-4"><div class="px-6 py-4 text-xs font-semibold">{{ $kota->kota_kode }}</div></td>
                            <td class="px-6 py-4 text-xs">{{ $kota->kota_nama }}</td>
                            <td class="px-6 py-4 text-xs text-gray-600">
                                {{ $kota->propinsi ? $kota->propinsi->propinsi_nama : '-' }}
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex flex-col md:flex-row justify-center gap-2">
                                    @if(hasMenuAccess(14, 'mru'))
                                    <a href="{{ url('/kota/edit') }}/{{ $kota->id }}"
                                    ...
                                    <a href="{{ route('kota.delete', $kota->id) }}"
