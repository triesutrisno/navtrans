...
@section('title')
    ...
    > Ubah Data
@endsection
@section('subtitle', 'Halaman ini digunakan untuk melakukan ubah data master kota kedalam sistem')
@section('content')

<div class="p-3 md:p-6">
    {{-- CARD --}}
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('kota.update', $data->id) }}"
              method="POST"
              class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <select name="propinsi_id" class="w-full rounded-md border border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200">
                    ...
                    @foreach($propinsi as $prop)
                        <option value="{{ $prop->id }}" {{ old('propinsi_id', $data->propinsi_id) == $prop->id ? 'selected' : '' }}>
                            {{ $prop->propinsi_nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text"
                       name="kota_kode"
                       value="{{ old('kota_kode', $data->kota_kode) }}"
                       maxlength="10"
            ...
            <div>
                ...
                <input type="text"
                       name="kota_nama"
                       value="{{ old('kota_nama', $data->kota_nama) }}"
                       maxlength="100"
                       class="w-full rounded-md border border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200">
