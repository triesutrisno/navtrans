...
@section('title')
    ...
    > Tambah Data
@endsection
@section('subtitle', 'Halaman ini digunakan untuk melakukan tambah data master customer kedalam sistem')
@section('content')

<div class="p-3 md:p-6">
    {{-- CARD --}}
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <form action="{{ url('/customer/store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <select name="customer_status" class="w-full rounded-md border border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200">
                    <option value="1" {{ old('customer_status') == '1' ? 'selected' : '' }}>Aktif</option>
                    <option value="2" {{ old('customer_status') == '2' ? 'selected' : '' }}>Tidak Aktif</option>
                </select>
            ...
            <div>
                ...
                <textarea name="customer_alamat"
                          rows="4"
                          class="w-full rounded-md border border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200">{{ old('customer_alamat') }}</textarea>
